...
export function aplicarParchesCORS() {
  ...
  // ============================================================
  // AUTO-RELOAD: Detecta cuando el servidor Vite se reinicia
  // y recarga la página automáticamente para evitar pantalla en blanco
  // ============================================================
  setupAutoReload();
}

/**
 * Detecta cuando el servidor Vite/frontend se reinicia y recarga la página.
 * Esto soluciona el problema de pantalla en blanco cuando el watchdog
 * reinicia el proceso node.exe.
 */
function setupAutoReload() {
  // Solo activar en modo desarrollo (cuando hay servidor Vite corriendo)
  // En producción no hay servidor de desarrollo
  ...
  const checkServer = () => {
    // Hacer un fetch simple al servidor para ver si responde
    ...
    .then(() => {
      if (serverWasDown && !reloadScheduled) {
        // El servidor volvió a estar disponible - recargar la página
        console.log('[AUTO-RELOAD] Servidor recuperado. Recargando página en 2 segundos...');
    ...
    .catch(() => {
      if (!serverWasDown) {
        console.warn('[AUTO-RELOAD] Servidor no disponible. Esperando recuperación...');
        serverWasDown = true;
      }
    });
  };

  // Verificar cada 5 segundos si el servidor está disponible
  checkInterval = setInterval(checkServer, 5000);

  // También detectar cuando la página pierde contenido (div root vacío)
  const checkRootEmpty = () => {
    const root = document.getElementById('root');
    if (root && root.children.length === 0 && !reloadScheduled) {
      console.warn('[AUTO-RELOAD] Contenido de la página vacío detectado. Recargando...');
      reloadScheduled = true;
      setTimeout(() => {
        window.location.reload();
      }, 1500);
  ...
  // Verificar el root cada 3 segundos
  setTimeout(() => {
    setInterval(checkRootEmpty, 3000);
  }, 5000); // Esperar 5s antes de empezar a verificar (para que React monte)

  console.log('[AUTO-RELOAD] Monitor de reconexión automática activado.');
}
