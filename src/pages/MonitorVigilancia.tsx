...
export default function MonitorVigilancia() {
  ...
  const { vigilantes, agregarVigilante, eliminarVigilante, actualizarVigilante, obtenerVigilantesConTransicion } = useVigilantes();
  ...
  useEffect(() => {
    if (solicitudesPendientes.length > prevPendientesRef.current) sonarNuevaSolicitud();
  ...
  const handleDevolver = (solicitudId: string, vigilante: string) => {
    ...
    toast({ title: "Llave devuelta", description: `${solicitud.lugar.nombre} recibida por ${vigilante}` });
  };
  ...
  const handleUndo = (solicitudId: string) => {
    ...
    if (success) {
      toast({ title: "Acción deshecha", description: "La operación fue revertida correctamente" });
    } else {
      toast({ title: "Error", description: "No se pudo deshacer la acción (tiempo expirado)", variant: "destructive" });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-card border-b px-6 py-2">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <SoundControls 
            ...
            onToggleMuteNuevaSolicitud={toggleMuteNuevaSolicitud}
            onToggleMuteEntregaDevolucion={toggleMuteEntregaDevolucion}
                ...
                {!isConnected ? (
                  <>
                    ...
                    <Button 
                      variant="outline" 
                      size="sm" 
                      className="gap-2 ml-2 bg-destructive/10 border-destructive/30 text-destructive hover:bg-destructive/20"
      ...
      <KeyManagementModal open={keyModalOpen} onOpenChange={setKeyModalOpen} lugares={lugaresDisponibles} onAgregarLlave={agregarLlave} onQuitarLlave={quitarLlave} onModificarLlave={modificarLlave} />
      <GuardManagementModal open={guardModalOpen} onOpenChange={setGuardModalOpen} vigilantes={vigilantes} onAgregar={agregarVigilante} onEliminar={eliminarVigilante} onActualizar={actualizarVigilante} />
      <ConfigurationModal open={configModalOpen} onOpenChange={setConfigModalOpen} configuracion={configuracion} onGuardar={actualizarConfiguracion} onResetear={resetearConfiguracion} />
      <KeyHistorySearch open={historySearchOpen} onOpenChange={setHistorySearchOpen} />
      ...
      <ObjetosOlvidadosModal
        ...
        onRegistrarClick={() => { setObjetosOpen(false); setRegistroObjetoOpen(true); }}
        onDevolver={devolverObjeto}
        buscarObjetos={buscarObjetos}
      />

      <RegistroObjetoModal
        open={registroObjetoOpen}
        onOpenChange={(v) => { setRegistroObjetoOpen(v); if (!v) setObjetosOpen(true); }}
        vigilantes={vigilantesActuales.map(v => v.nombre)}
        ...
        <section>
          ...
          {isLoading ? (
            <Card className="p-8 text-center">
              ...
              <h3 className="text-lg font-semibold mb-1">Cargando datos</h3>
              <p className="text-muted-foreground text-sm">Espere mientras se actualizan las solicitudes</p>
            </Card>
          ) : solicitudesPendientes.length === 0 ? (
            <Card className="p-8 text-center">
              ...
              <p className="text-muted-foreground text-sm">Las nuevas solicitudes aparecerán aquí automáticamente</p>
            </Card>
          ) : (
        ...
        <section>
          ...
          {isLoading ? (
            <Card className="p-8 text-center">
              <Loader2 className="w-12 h-12 mx-auto text-primary animate-spin mb-3" />
              <h3 className="text-lg font-semibold mb-1">Cargando datos</h3>
              <p className="text-muted-foreground text-sm">Espere mientras se actualizan las llaves en uso</p>
            </Card>
          ) : solicitudesEntregadas.length === 0 ? (
            <Card className="p-8 text-center">
        ...
        {solicitudesDevueltas.length > 0 && (
          <section>
            <div className="flex items-center gap-3 mb-4">
              ...
              <Badge className="bg-emerald-100 text-emerald-800 border-emerald-200">{solicitudesDevueltas.length} devuelta{solicitudesDevueltas.length !== 1 ? 's' : ''}</Badge>
            </div>
            <div className="space-y-2">
              {solicitudesDevueltas
                .sort((a, b) => {
                  // Ordenar por hora de devolución, más reciente primero
                  const fechaA = a.horaDevolucion ? new Date(a.horaDevolucion).getTime() : 0;
                  const fechaB = b.horaDevolucion ? new Date(b.horaDevolucion).getTime() : 0;
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-sm text-muted-foreground ml-2">Devuelta por {solicitud.usuario.nombre}</span>
                        {solicitud.recibidoPor && <span className="text-sm text-muted-foreground"> • Recibida por {solicitud.recibidoPor}</span>}
                      </div>
                    </div>
                    {solicitud.horaDevolucion && (
                      <span className="text-sm text-emerald-700">
                        {new Date(solicitud.horaDevolucion).toLocaleTimeString('es-UY', { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    )}
                  </div>
                </Card>
