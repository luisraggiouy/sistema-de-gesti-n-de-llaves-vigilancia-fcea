@echo off
REM =====================================================================
REM  REINSTALAR COMPLETO EL SISTEMA DE LLAVES FCEA  (v2.1 - anti-cierre)
REM  Borra todo lo viejo y reinstala desde cero. Si el .ps1 truena, la
REM  ventana NO cierra al segundo: muestra el error y espera Enter.
REM =====================================================================
title REINSTALAR COMPLETO SISTEMA DE LLAVES FCEA v2.1
color 1F

REM Verificar privilegios de administrador
net session >nul 2>&1
if %errorLevel% neq 0 (
    echo Solicitando permisos de administrador...
    powershell -NoProfile -Command "Start-Process -FilePath '%~f0' -Verb RunAs"
    exit /b
)

cd /d "%~dp0"
set "PS1=%~dp0lib\REINSTALAR-COMPLETO.ps1"

if not exist "%PS1%" (
    echo [ERROR CRITICO] No se encontro %PS1%
    pause
    exit /b 1
)

REM Try/catch externo: si el .ps1 lanza una excepcion no capturada,
REM la mostramos en pantalla en lugar de cerrar la ventana.
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "& { try { & '%PS1%' } catch { Write-Host ''; Write-Host '=== EXCEPCION NO CAPTURADA EN EL PS1 ===' -ForegroundColor Red; Write-Host $_.Exception.Message -ForegroundColor Red; Write-Host $_.ScriptStackTrace -ForegroundColor DarkRed; exit 99 } }"
set "RC=%ERRORLEVEL%"

echo.
echo =====================================================================
if "%RC%"=="0" (echo  El script termino normalmente.) else (echo  El script termino con codigo: %RC%)
echo =====================================================================
echo.
echo Presione una tecla para cerrar...
pause >nul
