# Script de Recuperación de Datos - Sistema de Gestión de Llaves FCEA
...
function MostrarEncabezado {
  ...
  SISTEMA DE GESTIÓN DE LLAVES FCEA - RECUPERACIÓN DE DATOS v1.0
====================================================================="
    Write-Output ""
}

function DetenerServicioPocketbase {
    Write-Output "Verificando si PocketBase está en ejecución..."
    ...
    if ($procesos) {
        ...
        Write-ColorOutput Green "✓ PocketBase detenido correctamente."
    } else {
        Write-Output "PocketBase no está en ejecución."
    }
}

function IniciarServicioPocketbase {
    Write-Output "`nIniciando PocketBase..."
    Start-Process -FilePath (Join-Path $rutaPocketbase "pocketbase.exe") -ArgumentList "serve" -WindowStyle Hidden
    Write-ColorOutput Green "✓ PocketBase iniciado correctamente.`n"
}
...
function ListarRespaldosDisponibles {
    ...
    Write-Output "`n--- RESPALDOS AUTOMÁTICOS ---"
    if ($respaldosAutomaticos.Count -gt 0) {
        Write-ColorOutput Green "Se encontraron $($respaldosAutomaticos.Count) respaldos automáticos:`n"
        ...
        Write-ColorOutput Yellow "No se encontraron respaldos automáticos."
    }
    
    Write-Output "`n--- RESPALDOS EN UNIDADES EXTERNAS ---"
    if ($respaldosManuales.Count -gt 0) {
        Write-ColorOutput Green "Se encontraron $($respaldosManuales.Count) respaldos externos:`n"
...
function RestaurarUltimoRespaldo {
    ...
    Write-Output "`n`nSe restaurará el respaldo más reciente:"
    Write-ColorOutput Yellow "- $($ultimoRespaldo.Name) ($($ultimoRespaldo.LastWriteTime))"
    
    $confirmacion = Read-Host "¿Confirma esta operación? (S/N)"
...
function RestaurarRespaldoSeleccionado {
    ...
    Write-Output "`n"
    $seleccion = Read-Host "Ingrese el número del respaldo a restaurar (1-$($todosRespaldos.Count))"
    
    try {
        $indice = [int]$seleccion - 1
        if ($indice -lt 0 -or $indice -ge $todosRespaldos.Count) {
            throw "Selección fuera de rango"
        }
        
        $respaldoSeleccionado = $todosRespaldos[$indice]
        
        Write-Output "`nSe restaurará el respaldo:"
        ...
        Write-ColorOutput Red "Error: Selección inválida."
    }
}

function RestaurarRespaldo($respaldoObj) {
    Write-Output "`n--- INICIANDO RESTAURACIÓN ---"
    ...
    try {
        ...
        Write-ColorOutput Green "✓ Restauración completada exitosamente."
        
        # Iniciar PocketBase nuevamente
        IniciarServicioPocketbase
        
        Write-Output "Restauración completada. El sistema ya puede utilizarse
con los datos restaurados desde el respaldo $($respaldoObj.Name)."
        
    } catch {
        Write-ColorOutput Red "Error durante la restauración: $_"
    }
}

function RestaurarBotonEmergencia {
    # Esta función intenta reconstruir la base de datos a partir de los datos existentes
    # e información del repositorio. Es un último recurso.
    
    Write-ColorOutput Red "`n¡ADVERTENCIA! Este es un procedimiento de emergencia que intentará
reconstruir la base de datos con los usuarios y llaves originales.
Solo debe usarse si todos los respaldos fallan.`n"
    
    $confirmacion = Read-Host "¿Está seguro que desea continuar? (S/N)"
    if ($confirmacion -ne "S") {
        Write-ColorOutput Yellow "Operación cancelada."
        return
    }
    ...
    try {
        ...
        if (Test-Path $rutaDatosIniciales) {
            ...
            Write-ColorOutput Green "✓ Datos iniciales restaurados."
        } else {
            Write-ColorOutput Yellow "No se encontraron datos iniciales en el repositorio."
            Write-Output "Iniciando restauración manual de estructuras mínimas..."
            
            # Recrear datos mínimos
        ...
        Write-ColorOutput Green "`n✓ Procedimiento de emergencia completado.
Los datos básicos han sido restaurados. Se ejecutarán las migraciones
cuando inicie PocketBase.`n"
        
        # Iniciar PocketBase nuevamente
        IniciarServicioPocketbase
        
        Write-Output "Procedimiento de emergencia completado. Ahora debe:
1. Acceder al sistema y verificar que funciona correctamente
2. Para restaurar los DATOS REALES del sistema (16 vigilantes y 161 llaves
   con sus ubicaciones), ejecute desde la raíz del proyecto:
       RESTAURAR_DATOS_URGENTE.bat
   Esto recreara automáticamente:
       - Vigilantes Matutino:   Sylvia (Jefa), Claudia, Laura, Lourdes, Luis, Dahiana
       - Vigilantes Vespertino: Martín (Jefe), Daniel, Nathia, Silvia, Alejandro, Caterin
       - Vigilantes Nocturno:   Gustavo (Jefe), Mario, Silvana, Fernando
       - Las 161 llaves del Tablero Principal con sus posiciones reales
3. Los usuarios solicitantes (docentes, alumnos, TAS) se cargan progresivamente
   al usar el sistema desde la terminal de usuario."
...
function MostrarMenu {
    MostrarEncabezado
    Write-Output "MENÚ PRINCIPAL
------------------
1. Listar respaldos disponibles
2. Restaurar último respaldo (más reciente)
3. Seleccionar respaldo específico para restaurar
4. Restauración de emergencia (último recurso)
...
"
    ...
    switch ($opcion) {
        ...
        "6" { 
            MostrarEncabezado
            Write-Output "¡Gracias por utilizar el Sistema de Recuperación!"
            Write-Output "Asegúrese de que PocketBase esté ejecutándose antes de usar el sistema."
            Write-Output ""
            break
        }
        default {
            Write-ColorOutput Red "Opción inválida. Intente nuevamente."
