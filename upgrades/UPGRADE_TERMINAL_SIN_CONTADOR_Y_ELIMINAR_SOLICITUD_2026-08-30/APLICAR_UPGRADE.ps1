...
#   Cambios:
#   1) TERMINAL (A/B): se quito la pantalla de exito con cuenta regresiva
#      de 5s (su boton "Cancelar Pedido" NO borraba realmente la solicitud,
#      solo confundia). Ahora, al enviar el pedido, aparece un aviso breve
#      y la Terminal vuelve INMEDIATAMENTE al inicio limpio para el proximo
#      usuario.
#   2) MONITOR: la ventana para "Deshacer" una entrega baja de 2 min a 1 min.
#   3) MONITOR: nuevo boton "Eliminar solicitud" (papelera) en cada solicitud
#      PENDIENTE. Con confirmacion, borra por completo el pedido equivocado
#      para que la persona pueda volver a solicitar la llave correcta.
# Fecha: 2026-08-30
#
# QUE HACE:
...
$Host.UI.RawUI.WindowTitle = 'UPGRADE Terminal sin contador + Eliminar solicitud - Sistema FCEA'
...
Line
Write-Host "  UPGRADE: Terminal sin contador + Eliminar solicitud en Monitor" -ForegroundColor Yellow
...
Write-Host "    1) Identificate y pedi una llave -> Confirmar." -ForegroundColor Green
Write-Host "    2) YA NO debe aparecer la cuenta regresiva de 5s: sale un aviso" -ForegroundColor Green
Write-Host "       breve y la Terminal vuelve sola al inicio, lista para otro usuario." -ForegroundColor Green
Write-Host "   MONITOR:" -ForegroundColor Green
Write-Host "    3) En la solicitud pendiente hay un boton rojo 'Eliminar solicitud'." -ForegroundColor Green
Write-Host "       Al tocarlo pide confirmacion y borra el pedido por completo." -ForegroundColor Green
Write-Host "    4) Tras ENTREGAR una llave, el 'Deshacer' ahora dura 1 minuto (antes 2)." -ForegroundColor Green
