...
#   Terminal de Usuario (A/B): al buscar una llave (ej. tecleando
#   "rendi") se despliega la lista de resultados. Tras SOLICITAR la
#   llave, el usuario se deslogueaba pero la caja de texto seguia con
#   "rendi" y la lista desplegada; habia que apretar F5 para dejarla
#   limpia. Ahora, tras enviar la solicitud, el buscador se limpia
#   automaticamente (texto y filtros) y la lista se repliega, dejando
#   la Terminal 100% limpia para el proximo usuario.
# Fecha: 2026-08-30
#
# QUE HACE:
...
$Host.UI.RawUI.WindowTitle = 'UPGRADE Terminal Limpia Buscador - Sistema FCEA'
...
Line
Write-Host "  UPGRADE: La Terminal limpia el buscador de llaves tras solicitar" -ForegroundColor Yellow
...
Write-Host "   1) Identificate como un usuario y en el buscador de llaves teclea" -ForegroundColor Green
Write-Host "      por ej. 'rendi' -> se despliega la lista de resultados." -ForegroundColor Green
Write-Host "   2) Selecciona una llave y envia la solicitud (Confirmar)." -ForegroundColor Green
Write-Host "   3) La Terminal debe volver al inicio con la sesion cerrada Y con el" -ForegroundColor Green
Write-Host "      buscador VACIO (sin 'rendi') y la lista replegada, SIN apretar F5." -ForegroundColor Green
