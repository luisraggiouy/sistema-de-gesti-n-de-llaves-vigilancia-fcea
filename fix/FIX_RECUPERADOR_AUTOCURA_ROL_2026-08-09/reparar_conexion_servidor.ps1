...
#
# NOVEDAD 2026-08-09 (autocura de rol):
#   Ademas de la IP, ahora REAFIRMA el campo "rol" segun el NOMBRE de la
#   PC (FCEA-TERMINAL-A -> terminal-a, etc). Motivo: el 09/08/2026 las 3
#   PCs quedaron con rol=terminal-b en dist\config.json (un upgrade viejo
#   piso el dist). El Recuperador arreglaba la IP pero dejaba el rol mal.
#   Ahora, al recuperar una terminal, tambien se autocorrige el rol.
# ============================================================
...
# ------------------------------------------------------------
# Helper (NUEVO 2026-08-09): deducir el rol de esta PC por el NOMBRE
# del equipo. Devuelve 'monitor'|'terminal-a'|'terminal-b'|'dashboard'
# o $null si el nombre no matchea ningun patron conocido.
# El orden importa: 'MONITOR' antes que 'TERMINAL'.
# ------------------------------------------------------------
function Get-RolPorHostname {
    $h = $env:COMPUTERNAME.ToUpper()
    switch -Regex ($h) {
        'MONITOR'          { return 'monitor' }
        'CABINA'           { return 'monitor' }
        'SERVIDOR'         { return 'monitor' }
        'TERMINAL[-_]?A'   { return 'terminal-a' }
        'TERMINAL[-_]?B'   { return 'terminal-b' }
...
# NUEVO 2026-08-09: deducir el rol de esta PC por el nombre del equipo.
# Si lo logramos (y NO es 'monitor'), se usa como AUTORIDAD para corregir
# el campo rol en los config.json. El caso 'monitor' se deja a la logica
# de salvaguarda historica (no forzamos monitor apuntando a IP remota).
$rolPorHost = Get-RolPorHostname
if ($rolPorHost) {
    Write-Ok "Rol segun nombre de PC ($env:COMPUTERNAME): $rolPorHost"
} else {
    Write-Warn "No pude deducir el rol por el nombre '$env:COMPUTERNAME'; mantengo el rol actual del config."
}
...
function Update-ConfigJson {
    ...
    try {
        ...
        # Autocura de rol (2026-08-09):
        #   - Si el nombre de la PC dice terminal-a/terminal-b/dashboard,
        #     ese valor es AUTORIDAD y corrige el rol si esta mal.
        #   - Si no pudimos deducirlo, caemos a la salvaguarda historica:
        #     rol='monitor' apuntando a IP remota es imposible -> terminal-a.
        if ($RolForzado -and $RolForzado -ne 'monitor') {
            if ($cfg.rol -ne $RolForzado) {
                Write-Warn "Corrigiendo rol '$($cfg.rol)' -> '$RolForzado' (segun nombre de PC)."
                $cfg.rol = $RolForzado
            }
        } elseif ($cfg.rol -eq 'monitor' -and $Ip -ne '127.0.0.1') {
...
#     la IP vieja.
Update-ConfigJson -Path (Join-Path $installDir 'public\config.json') -Ip $ipElegida -RolForzado $rolPorHost | Out-Null

# 1b) dist/config.json - lo que sirve el bundle al navegador.
Update-ConfigJson -Path (Join-Path $installDir 'dist\config.json') -Ip $ipElegida -RolForzado $rolPorHost | Out-Null
...
if (Test-Path $installCfgPath) {
    ...
    try {
        ...
        # Misma autocura de rol que en Update-ConfigJson.
        if ($rolPorHost -and $rolPorHost -ne 'monitor') {
            if ($cfg.rol -ne $rolPorHost) {
                Write-Warn "install_config: corrigiendo rol '$($cfg.rol)' -> '$rolPorHost'."
                $cfg.rol = $rolPorHost
            }
        } elseif ($cfg.rol -eq 'monitor' -and $ipElegida -ne '127.0.0.1') {
            $cfg.rol = 'terminal-a'
        }
