    ...
    if ($RolActual -eq "monitor") {
        ...
        if ($LastSeedDate) {
            ...
            if ($DaysSinceSeed -gt 90) {
                $HealthStatus.alerts += @{
                    level = "critical"
                    title = "Resguardo de datos en el pendrive muy desactualizado"
                    message = "Hace $DaysSinceSeed dias que no se actualizan los datos del pendrive de rescate. Si el sistema se rompe, la restauracion perderia $DaysSinceSeed dias de datos."
                    action = "Enchufe el pendrive de rescate en esta PC (Monitor) y ejecute ACTUALIZAR DATOS.bat desde el pendrive. Duracion: 30-60 segundos."
                    icon = "usb"
            ...
            elseif ($DaysSinceSeed -gt 45) {
                $HealthStatus.alerts += @{
                    level = "warning"
                    title = "Resguardo de datos en el pendrive desactualizado"
                    message = "Hace $DaysSinceSeed dias que no se actualizan los datos del pendrive de rescate. Se recomienda refrescarlos cada 45 dias."
                    action = "Enchufe el pendrive de rescate en esta PC (Monitor) y ejecute ACTUALIZAR DATOS.bat desde el pendrive. Duracion: 30-60 segundos."
                    icon = "usb"
                    documentation = "PLAYBOOK_MANTENIMIENTO.md"
                }
            ...
            if (-not $IsFreshInstall) {
                $HealthStatus.alerts += @{
                    level = "warning"
                    title = "Nunca se actualizaron los datos del pendrive de rescate"
                    message = "No hay registro de que se haya actualizado el pendrive de rescate en esta PC. Si el sistema se rompe, no habra backup portable disponible."
                    action = "Enchufe el pendrive de rescate en esta PC (Monitor) y ejecute ACTUALIZAR DATOS.bat desde el pendrive. Duracion: 30-60 segundos."
                    icon = "usb"
                    documentation = "PLAYBOOK_MANTENIMIENTO.md"
                }
