# =============================================================
# APLICAR_FIX.ps1  -  FIX LLAVES REALTIME (2026-08-06)  v2
#
# QUE ARREGLA:
#   Las llaves (coleccion 'lugares') que se agregan / editan /
#   eliminan en el Monitor NO aparecian en Terminal A / B hasta
#   reiniciar. Se agrego a SolicitudesContext.tsx suscripcion
#   realtime + polling de respaldo (mismo mecanismo ya probado
#   con usuarios), con upsert/borrado deduplicado por id.
#
# POR QUE v2 (NO compila en produccion):
#   El intento v1 reconstruia el frontend con 'vite build' en la
#   PC. Fallo porque el node_modules de produccion tiene versiones
#   incompatibles de @tanstack/react-query vs query-core
#   ("environmentManager is not exported"). El sistema anda porque
#   corre el dist PREBUILDEADO, pero un build nuevo ahi rompe.
#   -> Esta version NO compila: trae el dist YA COMPILADO desde la
#      laptop (con el fix) y lo instala, PRESERVANDO el config.json
#      y system_health.json propios de ESTA PC.
#   La config es 100% runtime (fetch /config.json), asi que un
#   unico dist sirve para los 3 roles (Monitor, Terminal A y B).
#
...
#   3) Reemplaza el contenido de dist\ por el dist_nuevo (con el fix).
#   4) Restaura config.json y system_health.json de esta PC.
#   5) (Opcional/consistencia) actualiza el fuente
#      src\contexts\SolicitudesContext.tsx (con backup).
...
$Host.UI.RawUI.WindowTitle = 'APLICAR FIX LLAVES REALTIME v2 - Sistema FCEA'

$INSTALL   = 'C:\sistema-llaves-fcea'
$DIST      = Join-Path $INSTALL 'dist'
$INDEX     = Join-Path $DIST 'index.html'
$DISTNUEVO = Join-Path $PSScriptRoot 'dist_nuevo'
$SRC       = Join-Path $INSTALL 'src\contexts\SolicitudesContext.tsx'
$SRCNUEVO  = Join-Path $PSScriptRoot 'SolicitudesContext.tsx'
...
$LOG = Join-Path $logDir ("LOG_APLICAR_FIX_LLAVES_{0}_{1}.log" -f $PC, $STAMP)
...
Header "APLICAR FIX LLAVES REALTIME v2 - $PC - $STAMP"
...
if ((Test-Path $SRCNUEVO) -and (Test-Path $SRC)) {
    try {
        ...
        Write-Host "  [OK] src\contexts\SolicitudesContext.tsx actualizado (backup .bak_$STAMP)." -ForegroundColor Green
    } catch {
...
Write-Host "   2) Verifica que la Terminal/Monitor levante normal e identifique usuarios." -ForegroundColor Cyan
Write-Host "   3) (Con las 3 PCs actualizadas) agrega/edita/borra una llave en el Monitor" -ForegroundColor Cyan
Write-Host "      y confirma que aparece/cambia SOLA en Terminal A y B." -ForegroundColor Cyan
Line
Write-Host ""
